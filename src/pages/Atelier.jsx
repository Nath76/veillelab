...
function ProvenanceChips({provenances=[]}){
 ...
 provenances.forEach((p,i)=>{
   const pub=p.publication_id||'Source'
   if(!groups.has(pub))groups.set(pub,{pages:[],timecodes:[],seen:new Set()})
   const g=groups.get(pub)
   const isTimecode=p.type==='timecode'||Boolean(p.timecode_debut)||Boolean(p.timecode_fin)
   const value=isTimecode?(p.label||[p.timecode_debut,p.timecode_fin].filter(Boolean).join('–')):String(p.page||p.label||'').trim()
   if(!value)return
   const key=`${isTimecode?'t':'p'}|${value}`
   if(g.seen.has(key))return
   g.seen.add(key)
   ;(isTimecode?g.timecodes:g.pages).push(value)
 })
 const labels=[]
 groups.forEach((g,pub)=>{
   if(g.pages.length)labels.push(`${pub} · ${g.pages.map((p,i)=>`${i===0?'p. ':'p. '}${p}`).join(' ; ')}`)
   if(g.timecodes.length)labels.push(`${pub} · ${g.timecodes.join(' ; ')}`)
 })
 if(!labels.length)return null
 return <div className="provenance-chips">{labels.map((label,i)=><span key={`${label}-${i}`}>{label}</span>)}</div>
}
