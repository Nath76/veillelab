...
export default function Expertises({data}){
  ...
  const entities=useMemo(()=>{
    const present=new Set(data.expertise_nodes.flatMap(n=>n.entities||[]))
    return ['Toutes les entités',...MINISTRY_ENTITIES.filter(x=>present.has(x)).sort((a,b)=>a.localeCompare(b,'fr'))]
  },[data])
  ...
  const overviewNodes=selectOverviewExpertises(filtered,60)
  const overviewIds=new Set(overviewNodes.map(n=>n.id)); const overviewLinks=visibleEdges.filter(e=>overviewIds.has(e.source)&&overviewIds.has(e.target)).slice(0,120)
  ...
  return <main className={`screen graph-screen expertise-screen ${selected?'has-drawer':''}`}>
    ...
    <section className="graph-workspace">
      <div className="workspace-toolbar"><div><h1>Carte des expertises ministérielles</h1><div className="big-count"><strong>{overviewNodes.length}</strong><span>nœuds · {Math.min(120,overviewLinks.length)} liens visibles</span><Icon name="info" size={17}/></div></div><div className="toolbar-actions"><button onClick={clear}><Icon name="reset" size={17}/>Réinitialiser</button><button onClick={()=>setFitToken(x=>x+1)}><Icon name="target" size={17}/>Ajuster à l’écran</button></div></div>
